...
export const VacanciesDisplay = ({ logged }: props) => {
  ...
  const [vacancyLevel, setVacancyLevel] = useState([
    "junior",
    "pleno",
    "senior",
  ]);
  ...
  function updateSearch(
    ...
    if (shouldPush) {
      ...
      whereToUpdate === "techsParam"
        ? aux.techsParam.splice(aux.techsParam.indexOf(whatValue as never), 1)
        : whereToUpdate === "vacancyType"
        ? aux.vacancyType.splice(aux.vacancyType.indexOf(whatValue as never), 1)
        : whereToUpdate === "workType"
        ? aux.workType.splice(aux.workType.indexOf(whatValue as never), 1)
        : whereToUpdate === "companySize"
        ? aux.companySize.splice(aux.companySize.indexOf(whatValue as never), 1)
        : aux.vacancyLevel.splice(
            aux.vacancyLevel.indexOf(whatValue as never),
            1
          );
    }
    ...
    <S.Container>
      ...
      <S.Display checked logged={isLogged}>
          ...
          {companySize.map((size, index) => {
            return (
              <Checkbox
                ...
                state={checked}
                key={index}
                stateType="companySize"
                handleMyValue={updateSearch}
              />
            );
          })}
          <MinimumDistanceSlider
            getSliderValues={getSliderValues}></MinimumDistanceSlider>
          <br />
        ...
        <div className="vacancies">
          ...
          <div className="grafic_box">
            <h3>
              Cidades que mais procuram por <span>React</span>
            </h3>
            <BarChartComponent />
          </div>
          <>
                ...
                <>
                  <div className="vacancy_box">
                    ...
                    <div className="vacancyInfo">
                      ...
                      <p>
                        faixa salarial: <span>{vacancy.wage}</span>
                      </p>
                      <MapPinLine color="#FBB04D" size={16} />
                      <p>
                        Localização: <span>{vacancy.location}</span>
                      </p>
                      <ComputerIcon color="#FBB04D" size={16} />
                      <p>
                        tipo de vaga: <span>{vacancy.vacancyType}</span>
                      </p>
                    </div>
